...
;;; TASK 3B ;;; 
;; The k-means clustering is an unsupervised variant of the Rocchio classifier. Where Rocchio
;; classification is reliant on the training data (supervised learning) to specify some
;; classes and class members, and thus the centroids from that, k-means clustering is a
;; clustering algorithms that defines centroids from random seeds. The algorithm is initialized
;; by computing centroids from the k number of seeds and each object is then assigned to the
;; cluster with the nearest centroid. After all the objects are assigned, new centroids
;; are computed based on the within-cluster sum of squares and the existing centroids are
;; 'moved' to their new location, so that they better represents their members. After this
;; is done, the objects are then again assigned to the centroid they are now closest
;; to (an object may be assigned to a new centroid). This process is done iteratively until
;; some termination criterion is met.

;; K-means clustering is thus similar to Rocchio classifier in that it uses centroid to
;; represent classes/clusters, and that the distance between an object and a centroid
;; determines class/cluster memberships (i.e objects are assigned to the nearest centroids).
;; They are different in that Rocchio classification has a more static centroid which is only
;; computed from the training data, while in k-means clustering is it continiously
;; recalculated until the stopping criterion is satisfied.


;;; TASK 3C ;;;
;; One method to evaluate our program's classifier decisions is to manually classify the
;; unclassified words (i.e. our 'unknown' words) ourselves, then run the program and compare
;; the programs classifications with our own. If the program was tasked to classify 10
;; words and got nine of them right, we could say that it was 90% successful. Other ways
;; we could compute an evaluation score could be to calculate accuracy, precision,
;; recall, F-score, but those measures are often better suited for scenarioes where we
;; can have more of a pass/fail situation with true positives, false positives, true
